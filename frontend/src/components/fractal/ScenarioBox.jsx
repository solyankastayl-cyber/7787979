...
function RangeStrip({ p10, p50, p90, basePrice }) {
  ...
  return (
    <div className="mt-4 px-2" data-testid="range-strip">
      {/* Visual bar */}
      <div className="relative h-2 bg-gradient-to-r from-red-200 via-slate-200 to-emerald-200 rounded-full">
        {/* P50 marker */}
        <div 
          className="absolute top-0 w-3 h-3 bg-blue-500 rounded-full -translate-x-1/2 -translate-y-[2px] ring-2 ring-white"
          style={{ left: `${Math.max(5, Math.min(95, p50Position))}%` }}
          title={`Base (P50): ${formatPrice(p50)}`}
        />
        {/* NOW marker */}
        <div 
          className="absolute top-0 w-2 h-2 bg-slate-600 rounded-full -translate-x-1/2"
          style={{ left: `${Math.max(5, Math.min(95, basePosition))}%` }}
          title={`NOW: ${formatPrice(basePrice)}`}
        />
      </div>
    ...
    <div className="mt-4 p-3 bg-slate-50 rounded-lg border border-slate-200" data-testid="outcome-stats">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
        ...
        <div>
          ...
          <div className="text-xs text-slate-500">Prob Up</div>
        </div>
        ...
        <div>
          ...
          <div className="text-xs text-slate-500">Avg Max DD</div>
        </div>
        ...
        <div>
          ...
          <div className="text-xs text-slate-500">Tail (P95)</div>
        </div>
